...
const rows = [
  { keyword: "putangina", language: "Bisaya", severityWeight: 0.9 },
  { keyword: "gago", language: "Filipino", severityWeight: 0.8 },
  { keyword: "stupid", language: "English", severityWeight: 0.7 },
  { keyword: "bobo", language: "Filipino", severityWeight: 0.6 },
];
      ...
      <div className={`${CARD} overflow-hidden`}>
        <div className="flex items-center gap-3 p-5 border-b border-gray-100 dark:border-white/10">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">UI-only table with static rows</p>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[640px]">
            ...
            <tbody>
              {rows.map((row) => (
                <tr
                  key={`${row.keyword}-${row.language}`}
                  className="border-b border-gray-50 dark:border-white/5 hover:bg-indigo-50/50 dark:hover:bg-white/5 transition-colors"
                >
                  <td className="py-3 px-4 text-gray-900 dark:text-white font-medium">{row.keyword}</td>
                  <td className="py-3 px-4 text-gray-500 dark:text-gray-400">{row.language}</td>
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="h-2 w-28 rounded-full bg-gray-200 dark:bg-white/10 overflow-hidden">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-purple-500"
                          style={{ width: `${row.severityWeight * 100}%` }}
                        />
                      </div>
                      <span className="text-gray-700 dark:text-gray-300 tabular-nums">{row.severityWeight.toFixed(1)}</span>
                    </div>
                  </td>
      ...
      <form onSubmit={handleSave} className={`${CARD} p-5`}>
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Static form interaction only</p>
          </div>
        </div>
        ...
        <button
          type="submit"
          className="mt-5 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-sm font-semibold hover:opacity-90 active:opacity-80 transition-opacity shadow-lg shadow-indigo-500/20"
        >
